...
async function semanticRelease () {
  ...
  var stdout = cp.execSync('./node_modules/.bin/semantic-release', childOpts).toString()
  process.stdout.write(stdout)
  var version = ''
  // locate the release version
  // ex:
  //  [Semantic release]: Published release: 0.0.1
  try {
    console.log(`[clipster:semantic-release]: searching for published version`)
    version = stdout.match(/Published release:\s*([^\s]*)\s*/)[1]
